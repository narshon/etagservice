...
class adLDAPGroups {
    ...
    */
    public function members($group, $recursive = NULL)
    {
        ...
        for ($i=0; $i<$users["count"]; $i++){
            //echo "here ",$users[$i], exit;
             $filter = "(&(objectCategory=person)(distinguishedName=" . $this->adldap->utilities()->ldapSlashes($users[$i]) . "))";
             $fields = array("samaccountname", "distinguishedname", "objectClass");
             $sr = ldap_search($this->adldap->getLdapConnection(), 'OU=Groups,OU=kemri-wtrp, DC=kwtrp, DC=org', $filter, $fields);
             $entries = ldap_get_entries($this->adldap->getLdapConnection(), $sr);
    ...
    */
    public function cn($gid){    
        ...
        /**
         * inhouse functions. Courtesy of NNgao.
         */
        ...
        function get_members($group) {
            ...
            foreach($entries[0]['member'] as $member) {
                if($dirty == 0) {
                    ...
                    $member_dets = $this->explode_dn($member); // print_r($member_dets); exit;
                    $username=str_replace("CN=","",$member_dets[0]);
                    $names=explode(' ',$username);
                    $givenName=$names[0];
                    $sn=$names[1];

                    //get the sAMAccountName of this user
