...
class ViewDetailsController extends Controller
{
	...
	{       
                $sc=new ServiceComponent($this,"ViewDetails", METADB_DB);
                $sc->showAll();
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['ViewDetails'];
			if($model->save()){
	 ...
	 */
	public function actionDelete($id,$safe)
	{       
                
		if(Yii::app()->request->isPostRequest || $safe=="safe")
		{
			...
			if(!isset($_GET['ajax']))
				$this->redirect(isset($_POST['returnUrl']) ? $_POST['returnUrl'] : array('service/admin'));
		}
		else
	...
	{
		$dataProvider=new CActiveDataProvider('ViewDetails');
	...
	{
		$model=new ViewDetails('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ViewDetails']))
			$model->attributes=$_GET['ViewDetails'];
	...
	{
		$model=ViewDetails::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='view-details-form')
		{
